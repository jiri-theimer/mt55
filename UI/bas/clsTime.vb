Namespace COM
    Public Class clsTime
        ...
        Public Function KolikZbyvaCasu(ByVal d As Date, ByVal intMaxPocetUrovni As Integer, ByVal bolShowZeroUnits As Boolean) As String
            Dim s As String = "", x As Integer = 0
            Dim dny As Long = DateDiff(DateInterval.Day, d, Now, , 0)
            If (dny = 0 And bolShowZeroUnits) Or dny <> 0 Then
                If x < intMaxPocetUrovni Then s = dny.ToString & "d"
                x += 1
            End If

            Dim hod As Long = DateDiff(DateInterval.Hour, d, Now, 0)
            ...
            If x < intMaxPocetUrovni Then
                Dim min As Long = DateDiff(DateInterval.Minute, d, Now, 0)
